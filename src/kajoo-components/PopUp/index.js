/* eslint-disable */
import React from 'react'
import { Container, Text, Image } from '@kajoo-ai/sitecore-react'

import PropTypes from 'prop-types'

import './style.css'

const PopUp = (props) => {
  return (
    <Container className={`pop-up-pop-up ${props.rootClassName} ${props.params?.styles} `}>
      <Container className="pop-up-button">
        <Container className="pop-up-cta">
          <Text text="Add to cart" className="pop-up-text" />
        </Container>
        <Container className="pop-up-frame-5">
          <Container className="pop-up-frame-11">
            <Image
              src="data:image/svg+xml;base64,"
              alt="gridiconsshareI117"
              className="pop-up-gridiconsshare"
            />
            <Text text="Share" className="pop-up-text1" />
          </Container>
          <Container className="pop-up-frame-12">
            <Image
              src="data:image/svg+xml;base64,"
              alt="comparesvgrepocom1I117"
              className="pop-up-comparesvgrepocom-1"
            />
            <Text text="Compare" className="pop-up-text2" />
          </Container>
          <Container className="pop-up-frame-10">
            <Image
              src="data:image/svg+xml;base64,"
              alt="HeartI117"
              className="pop-up-heart"
            />
            <Text text="Like" className="pop-up-text3" />
          </Container>
        </Container>
      </Container>
    </Container>
  )
}

PopUp.defaultProps = {
  rendering: {},
  rootClassName: '',
  fields: {},
}

PopUp.propTypes = {
  rendering: PropTypes.object,
  rootClassName: PropTypes.string,
  fields: PropTypes.object,
}

export default PopUp
